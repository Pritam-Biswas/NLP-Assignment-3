...
# seq = ['right_arc','left_arc']
#
# dep_idx = {}
# count = 0
# for i in dep_relations:
#     for j in seq:
#         dep_idx[(i,j)]=0
#         count+=1
#
# dep_idx[('shift',None)]=count
...
class FeatureExtractor(object):
    ...
    def get_encoding(self, arr, words, pos):
        arr_idx = []
        for i in arr:
            word = words[i]
            word_idx = self.word_vocab.get(word,None)
            if word_idx is None:
                pos_val = str(pos[i])
                if pos_val == "NNP":
                    arr_idx.append(self.word_vocab["<NNP>"])
                elif pos_val == "CD":
                    arr_idx.append(self.word_vocab["<CD>"])
                elif pos_val == "None":
                    arr_idx.append(self.word_vocab["<ROOT>"])
                else:
                    arr_idx.append(self.word_vocab["<UNK>"])
        return arr_idx
    ...
    def get_input_representation(self, words, pos, state):
        ...
        buffer_idx = self.get_encoding(buffer,words,pos)
        stack_idx = self.get_encoding(stack,words,pos)

        if len(stack_idx)<3:
            stack_idx.extend([null_idx]*(3-len(stack_idx)))
        if len(buffer_idx)<3:
            buffer_idx.extend([null_idx]*(3-len(buffer_idx)))

        stack_idx = np.array(stack_idx)
        buffer_idx = np.array(buffer_idx)

        return np.concatenate((stack_idx,buffer_idx))
        # return np.zeros(6)

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        label = extractor.output_labels[output_pair]
        return utils.to_categorical(label,num_classes=91)
        # pos_to_idx = self.pos_vocab
        # try:
        #     label = pos_to_idx.get(output_pair[1].upper(),pos_to_idx["<UNK>"])
        # except AttributeError:
        #     label = pos_to_idx["<NULL>"]
        # return utils.to_categorical(label,num_classes=91)
        # return np.zeros(91)
